...
export function clearTrialFormData(): void {
  try {
    ...
    console.warn('Failed to clear trial form data:', error);
  }
}
...
export const trialService = {
  ...
  async canStartTrial(): Promise<{ allowed: boolean; count: number }> {
    try {
      const { data, error } = await supabase.rpc('count_trial_churches');
  ...
  async getTrialInfo(churchId: string | undefined): Promise<TrialInfo> {
    ...
    try {
      const { data, error } = await supabase.rpc('get_trial_info', { p_church_id: churchId });
  ...
  async setInstitutionalCompleted(churchId: string): Promise<void> {
    const { error } = await supabase.rpc('set_institutional_completed', { p_church_id: churchId });
    if (error) throw error;
  },

  /** Salvar lead e expirar trial (chamado quando trial expira) */
  async saveLeadAndExpire(churchId: string): Promise<void> {
    const { error } = await supabase.rpc('save_trial_lead_and_expire', { p_church_id: churchId });
    if (error) throw error;
  },
  ...
  async createTrialChurchForUser(adminEmail: string, adminName: string): Promise<string> {
    ...
    await supabase.rpc('create_trial_subscription_for_church', { p_church_id: churchId });
    if (formData) {
      ...
      if (Object.keys(updates).length > 0) {
        try {
          await churchesService.update(churchId, updates);
        } catch (error) {
          console.warn('Failed to update church trial data:', error);
        }
      }
      clearTrialFormData();
    }
